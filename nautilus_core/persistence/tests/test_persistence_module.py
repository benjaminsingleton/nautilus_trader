...
def test_python_catalog_v2_single_stream():
    ...
    for chunk in result:
        ...
        # check sorted data across chunks
        for data in ticks:
            if last_ts_init is not None and data.ts_init < last_ts_init:
                assert False
            else:
                last_ts_init = data.ts_init
